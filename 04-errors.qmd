...
opts_privacy <- c(
"When the dataset has been previously published under open license",
answer = "When the dataset contains sensitive or confidential data, because these functions reveal actual values",
"When using RStudio instead of VS Code, because summary() behaves differently",
"When the dataset includes factors or characters that the AI cannot parse"
)

opts_prompt <- c(
"Provide only the output so the AI can infer what went wrong",
answer = "Provide the code, the intended outcome, and a short description of what seems incorrect",
"Ask the AI to rewrite the entire function from scratch to ensure correctness",
"Send only the error message since the code is functional"
)

opts_diff <- c(
"Because cognitive overload forces the learner to memorise the correct syntax",
answer = "Because effortful problem-solving creates desirable difficulties that strengthen retention and transfer of debugging skills",
"Because frustration reduces confidence and lowers the chance of repeating the same mistake",
"Because debugging replaces the need for structured practice or feedback"
)
...
1. When using AI for debugging, you should provide both the `r fitb(c("code"), ignore_case = TRUE)` and the `r fitb(c("error", "error message"), ignore_case = TRUE)` to help it accurately diagnose the problem.

>

2. When might summary() or str() be unsafe to share with AI?

`r longmcq(opts_privacy)`

>

3. What is the best strategy for prompting AI when your code runs but produces unexpected results?

`r longmcq(opts_prompt)`

>

4. According to self-determination theory, how can AI support autonomy during debugging?

`r mcq(c( "By providing step-by-step solutions before the user attempts a fix", answer = "By explaining rather than replacing reasoning, allowing the learner to choose when and how to seek help", "By automatically fixing code errors without user input", "By suggesting unrelated optimisations to expand the learner’s knowledge" ))`

>

5. Why is struggling briefly with a coding error beneficial for learning?
