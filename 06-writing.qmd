...
First, download the dataset from the [Open Science Framework](https://osf.io/a4zmv). Create a new Quarto or Rmd document and ensure the dataset is in the same folder, as this document, and then run the below code in a new code chunk. This will load in the data and then it will create the three objects using the code I have written and the approach I have taken so that we can compare with how Copilot gets on.
...
This integrity issue isn't just about students cheating on their homework. The scientific literature is already [full of AI garbage](https://retractionwatch.com/papers-and-peer-reviews-with-evidence-of-chatgpt-writing/). There will be medical "treatments" being prescribed based on AI fabricated/analysed data that at best don't work and at worst are harmful. Code written by AI is significantly increasing [security risks](https://www.techradar.com/pro/ai-is-creating-code-faster-but-this-also-means-more-potential-security-issues).
...
opts_security <- c("Copy-paste a few example rows into the chat so the model can infer variable types", "Use `summary()` or `str()` to describe the dataset to the AI, regardless of sensitivity", answer = "Start by providing only variable names (e.g., with `ls()`), scaling up detail only if necessary and safe", "Send the full CSV file because Copilot Enterprise does not store data")

opts_ethics <- c(
"It is acceptable because AI tools are considered standard assistive technology",
"It is fine if the final code runs correctly",
answer = "It raises transparency and authorship concerns because the analytical process cannot be audited or replicated",
"It only violates policy if the AI was paid for using institutional funds"
)
```


1. Why is it dangerous to assume that an AI coding assistant “knows” what it is doing?

`r mcq(c( "Because it might not have been trained on the most recent R syntax", answer = "Because it does not possess reasoning or understanding—its output is pattern prediction, not knowledge", "Because it sometimes generates syntax errors due to poor tokenisation", "Because it prefers certain libraries over others" ))`

>

2. When Copilot produced incorrect STARS mean scores, the root cause was that it calculated `r fitb(c("sums", "sum scores", "total scores"), ignore_case = TRUE)` instead of means.

>

3. You are working with a sensitive dataset that includes demographic and psychological measures. Which of the following is the most responsible approach when prompting an AI model?

`r longmcq(opts_security)`

>

4. Which psychological mechanism is most strengthened when you work through code rather than accept AI output uncritically?
`r mcq(c( "Automaticity from repeated syntax exposure", answer = "Mastery experiences that build self-efficacy and competence", "Transfer of learning through imitation", "Cognitive load reduction through delegation" ))`

>

5. A student uses AI to generate R code for their dissertation dataset, but does not document what the AI produced versus what they wrote. Which statement best describes the issue?
